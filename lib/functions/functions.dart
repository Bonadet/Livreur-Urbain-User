      ...
      if (value['drivers'] != null) {
        ...
        drivers.forEach((k, v) {
          if (outStationDriver
              ...
              .isEmpty) {
            outStationDriver.add({'id': key, 'driver': k, 'price': v['price']});
            ...
            RemoteNotification noti = RemoteNotification(
                title: 'Got new driver',
                body:
                    '${v['driver_name']} bid for ride amount of ${v['price']}');
            showRideNotification(noti);
          } else {
            if (outStationDriver.firstWhere(
                    (e) => e['id'] == key && e['driver'] == k)['price'] !=
                v['price']) {
              outStationDriver.firstWhere(
                      (e) => e['id'] == key && e['driver'] == k)['price'] =
                  v['price'];
              RemoteNotification noti = RemoteNotification(
                  title: 'bid amount updated',
                  body:
                      '${v['driver_name']} bid updated for ride amount of ${v['price']}');
              showRideNotification(noti);
              // pref.setString('outstationpush', json.encode(outStationDriver));
              saveListToPrefs(outStationDriver);
            }
