...
class _LoadingPageState extends State<LoadingPage> {
  ...
  navigate() {
    if (userRequestData.isNotEmpty && userRequestData['is_completed'] == 1) {
      ...
      //searching ride page
      if (userDetails['metaRequest']['data']['is_rental'] == true) {
        addressList.add(AddressList(
            id: '1',
            type: 'pickup',
            address: userDetails['metaRequest']['data']['pick_address'],
            pickup: true,
            latlng: LatLng(userDetails['metaRequest']['data']['pick_lat'],
                userDetails['metaRequest']['data']['pick_lng']),
        ...
        addressList.add(AddressList(
            ...
            name: userDetails['name'],
            number: userDetails['mobile']));
        if (userDetails['metaRequest']['data']['requestStops']['data'] != []) {
          for (var i = 0;
              i <
              ...
              i++) {
            addressList.add(AddressList(
                id: userDetails['metaRequest']['data']['requestStops']['data']
                        [i]['id']
                    .toString(),
                type: 'drop',
                address: userDetails['metaRequest']['data']['requestStops']
                    ['data'][i]['address'],
                latlng: LatLng(
                    userDetails['metaRequest']['data']['requestStops']['data']
                        [i]['latitude'],
                    userDetails['metaRequest']['data']['requestStops']['data']
                        [i]['longitude']),
                name: userDetails['metaRequest']['data']['requestStops']['data']
                        [i]['poc_name'] ??
                    '',
                number: userDetails['metaRequest']['data']['requestStops']['data'][i]['poc_name'] ?? '',
                instructions: userDetails['metaRequest']['data']['requestStops']['data'][i]['poc_instruction'],
                pickup: false));
          }
        }
        Navigator.pushAndRemoveUntil(
